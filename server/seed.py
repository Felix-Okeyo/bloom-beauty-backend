...
with app.app_context():
    ...
    def seed_data():
        ...
        # Query for existing categories and brands
        category_1 = Category.query.filter_by(cat_name="Lipstick").first()
        category_2 = Category.query.filter_by(cat_name="Face").first()
        category_3 = Category.query.filter_by(cat_name="Eyes").first()
        category_4 = Category.query.filter_by(cat_name="Make-up").first()

        brand_1 = Brand.query.filter_by(brand_name="Huddah Cosmetics").first()
        brand_2 = Brand.query.filter_by(brand_name="Rihanna").first()
        brand_3 = Brand.query.filter_by(brand_name="Maybelline").first()
        brand_4 = Brand.query.filter_by(brand_name="MAC Cosmetics").first()

        # Create product instances using the existing categories and brands
        product_1 = Product(image="https://www.pexels.com/photo/close-up-photo-of-person-holding-lipstick-3060257/", p_name="Huddah Lipstick - Red Velvet", description="High-quality red lipstick that provides a smooth and long-lasting finish. Perfect for any occasion.", price=200, category=1, brand=1)
        product_2 = Product(image="https://www.pexels.com/photo/venus-mascara-2697787/", p_name="Rihanna Mascara - Volume Boost", description="Achieve voluminous lashes with Rihanna's mascara. This mascara lifts, separates, and adds volume for a dramatic look.", price=100, category=1, brand=2)
        product_3 = Product(image="https://www.pexels.com/photo/makeup-palette-on-black-wooden-table-4620874/", p_name="Urban Chic Eyeshadow Palette", description="Explore a variety of shades with this eyeshadow palette. From subtle neutrals to bold colors, create endless eye looks.", price=250, category=3, brand=1)
        product_4 = Product(image="https://www.pexels.com/photo/assorted-makeup-brush-set-7290669/", p_name="Luxury Makeup Brush Set", description="Upgrade your makeup routine with this luxurious brush set. The soft bristles and ergonomic handles ensure a flawless application.", price=150, category=4, brand=2)
        product_5 = Product(image="https://www.pexels.com/photo/a-variety-of-lipglosses-on-a-pink-background-15854300/", p_name="Maybelline Lip Gloss - Pink Delight", description="Shiny and moisturizing lip gloss by Maybelline. Add a pop of color and shine to your lips with this Pink Delight shade.", price=120, category=1, brand=3)
        product_6 = Product(image="https://www.pexels.com/photo/beauty-blush-brush-color-354962/", p_name="MAC Blush Brush", description="Sculpt and define your cheeks with this MAC blush brush. Soft and angled bristles make application easy and precise.", price=50, category=4, brand=4)
        product_7 = Product(image="https://www.pexels.com/photo/close-up-photo-of-pink-lipstick-and-blush-on-2533266/", p_name="Maybelline Lipstick - Coral Crush", description="Vibrant coral lipstick by Maybelline. Provides a creamy texture and bold color payoff.", price=180, category=1, brand=3)
        product_8 = Product(image="https://www.pexels.com/photo/studio-shot-of-foundation-in-glass-container-5403543/", p_name="MAC Foundation - Natural Glow", description="Lightweight foundation for a natural glow. Blends seamlessly and provides all-day coverage.", price=280, category=2, brand=4)
        product_9 = Product(image="https://www.pexels.com/photo/skin-care-tools-set-photo-8015871/", p_name="Rihanna Face Cream - Hydrating Moisture", description="Hydrating face cream by Rihanna. Infused with moisturizing ingredients for soft and supple skin.", price=220, category=2, brand=2)
        product_10 = Product(image="https://www.pexels.com/photo/powder-and-concealer-on-tray-7670761/", p_name="MAC Concealer - Full Coverage", description="Full coverage concealer by MAC. Conceals imperfections and brightens the under-eye area.", price=150, category=2, brand=4)
        ...
        product_12 = Product(image="https://www.pexels.com/photo/makeup-brush-on-black-container-1115128/", p_name="Huddah Makeup Brush Set", description="High-quality makeup brush set by Huddah Cosmetics. Includes brushes for eyes, face, and lips.", price=200, category=4, brand=1)
